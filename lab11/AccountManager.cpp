...
void AccountManager::changeUserName(const string& oldUserName, const string& newUserName) {
	...
	if (it != myAccounts.end()) {
		it = myAccounts.find(newUserName);
		if (it == myAccounts.end()) {
			it = myAccounts.find(oldUserName);
			createNewAccount(newUserName, it->second);
...
void AccountManager::deleteAccount(const string& userName) {
	...
	if (it != myAccounts.end()) {
		...
		throw Exception("deleteAccount()", "Cannot find specified account.");
	}
...
/* Utility method to change password of a user
 * Parameter: userName, a string
 * Parameter: newPassword, a string
 * Precondition: userName is in myAccounts.
 * Postcondition: the (userName, password) pair has been changed to
...
void AccountManager::changePassword(const string& userName,
	...
	if (it != myAccounts.end()) {
		...
		throw Exception("changePassword()", "Cannot find specified username");
	}
}
